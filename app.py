...
st.sidebar.markdown("---")
st.sidebar.caption("💡 **Regola del Salvadanaio:**\nL'algoritmo preleva l'extra dal salvadanaio solo se l'ETF scelto si trova realmente in storno tecnico (sotto la SMA20 e con RSI < 45).")
...
def fetch_etf_data():
    ...
    for symbol, info in ETF_CONFIG.items():
        try:
            ...
            # Logica segnale basata su storno effettivo
...
# --- LOGICA DI SELEZIONE E ALLOCAZIONE SICURA ---
...
# Regola di spesa intelligente e protettiva
if best_dist_sma < 0 and best_rsi < 45 and best_etf_signal == "Strong Buy":
    extra_invested = savings_cash
    total_investable = monthly_budget + extra_invested
elif best_etf_signal == "Accumulate":
    extra_invested = 0
    total_investable = monthly_budget
else:
    # Se il mercato è sopra la media o l'RSI non giustifica lo storno, riduciamo o salvaguardiamo la liquidità
    extra_invested = 0
    total_investable = monthly_budget * 0.5
...
with col_title:
    ...
    st.write("Acquisto mensile basato su storno effettivo sotto la media mobile e RSI di controllo.")
...
with kpi2:
    ...
    <div class="metric-card">
        ...
        <div class="metric-subtitle">{"Prelievo extra attivato!" if extra_invested > 0 else "In attesa di storno reale"}</div>
    </div>
    """, unsafe_allow_html=True)
...
st.subheader(f"💶 Assegnazione Budget: €{total_investable:.2f} su {selected_etf_name}")
...
<div class="insight-box">
    📊 <b>Logica di Allocazione Intelligente:</b> 
    Il budget viene allocato su <b>{selected_etf_name}</b> con il controllo combinato di distanza dalla SMA20 e RSI. Se il mercato non è in effettivo storno tecnico, il sistema modula la spesa proteggendo la liquidità.
</div>
""", unsafe_allow_html=True)

st.caption("Source: Yahoo Finance Live Data · Developed Markets PAC Screener | QuantEdge v4.1")