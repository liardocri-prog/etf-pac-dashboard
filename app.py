...
# 2. CSS Personalizzato (Dark Theme Globale & Modali)
st.markdown("""
<style>
    /* Sfondo Generale */
    .stApp {
...
for item in raw_data:
    ...
    # Valore numerico del drawdown per ordinare e trovare il più conveniente
    item["DD_Val"] = float(item["Drawdown"].replace('%', ''))
    data.append(item)

df = pd.DataFrame(data)

# --- LOGICA DI SELEZIONE DELL'UNICA AZIONE PIÙ CONVENIENTE DEL MESE ---
# Ordiniamo per Drawdown crescente (il più negativo / scontato è in cima)
df = df.sort_values(by="DD_Val", ascending=True).reset_index(drop=True)

# L'ETF migliore del mese è il primo in classifica
best_etf_idx = 0
best_etf_signal = df.loc[best_etf_idx, "Signal"]

# Se l'ETF più conveniente è in Strong Buy, sblocchiamo il salvadanaio
extra_invested = savings_cash if best_etf_signal == "Strong Buy" else 0
total_investable = monthly_budget + extra_invested

# Assegniamo TUTTO il budget all'ETF più conveniente e 0 agli altri
df["Allocazione_Euro"] = 0.0
df.loc[best_etf_idx, "Allocazione_Euro"] = total_investable

# --- FUNZIONE GENERAZIONE GRAFICI (CON TESTI CHIARI) ---
def generate_etf_charts(etf_symbol, seed_val):
    ...
    fig_price.update_layout(
        ...
        plot_bgcolor="#0d1117",
        font=dict(color="#e2e8f0"), # Testi chiari visibili
...
with col_title:
    ...
    st.write("Acquisto mensile concentrato **sull'ETF più conveniente** (maggiore sconto / drawdown). Clicca su un titolo per aprire il pop-up.")
...
# --- RIPARTIZIONE IMPORTO EURO (UNICA SCELTA) ---
...
st.caption("Source: Simulated market data · Developed Markets PAC Screener | QuantEdge v3.0")