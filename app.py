...
savings_cash = st.sidebar.number_input(
    ...
    value=150, 
    step=10,
    help="Soldi extra pronti a essere sbloccati se lo storno è forte."
)

st.sidebar.markdown("---")
st.sidebar.caption("💡 **Allocazione Ponderata SMA20 & RSI:**\nL'importo investito cresce proporzionalmente quanto più l'ETF si trova sotto la sua media mobile e con RSI scarico.")
...
def fetch_etf_data():
    ...
    for symbol, info in ETF_CONFIG.items():
        try:
            ...
            # Media Mobile a 20 periodi (SMA 20)
            sma_20 = hist['Close'].rolling(window=20).mean()
            cur_sma_20 = sma_20.iloc[-1]
            dist_sma = ((current_price - cur_sma_20) / cur_sma_20) * 100
            ...
            # Segnale descrittivo
            sotto_media = current_price < cur_sma_20
            a_ribasso = (macd_status == "Bearish") or (change_pct < 0)

            if sotto_media and a_ribasso and rsi_val <= 45:
                signal = "Strong Buy"
            elif not sotto_media and macd_status == "Bullish":
                signal = "Wait"
            else:
                signal = "Accumulate"
...
# --- LOGICA DI SELEZIONE E PESATURA DINAMICA (SMA20 + RSI) ---
df = df.sort_values(by="Dist_SMA", ascending=True).reset_index(drop=True)
best_etf_idx = 0
best_etf = df.loc[best_etf_idx]
best_etf_signal = best_etf["Signal"]
best_rsi = float(best_etf["RSI"])
best_dist_sma = best_etf["Dist_SMA"]

# Calcolo dinamico dell'importo basato sul peso di Dist_SMA e RSI
if best_dist_sma < 0:
    # Sotto la media: più è profondo lo storno e più l'RSI è basso, più budget e salvadanaio vengono sbloccati
    depth_factor = min(abs(best_dist_sma) / 4.0, 1.0) # Satura a -4% di distanza
    rsi_factor = max(0, (45 - best_rsi) / 15.0) if best_rsi < 45 else 0.0
    
    extra_bonus = savings_cash * ((depth_factor + rsi_factor) / 2.0)
    total_investable = monthly_budget + extra_bonus
    extra_invested = extra_bonus
else:
    # Sopra la media: riduciamo l'investimento proporzionalmente a quanto il prezzo è sopra la SMA20
    over_penalty = min(best_dist_sma / 3.0, 1.0) # Satura a +3% sopra la media
    total_investable = monthly_budget * max(0.2, (1.0 - over_penalty))
    extra_invested = 0.0
...
def show_etf_popup(etf_info):
    ...
    p2.metric("Distanza SMA 20", f"{etf_info['Dist_SMA']:.2f}%")
    p3.metric("RSI", etf_info["RSI"])
...
with col_title:
    ...
    st.write("Allocazione dinamica ponderata su Distanza SMA20 e RSI.")
...
# --- TOP CARDS (KPIs) ---
selected_etf_name = best_etf["ETF"]
...
with kpi1:
    ...
    <div class="metric-card">
        ...
        <div class="metric-subtitle">Ponderato su {selected_etf_name}</div>
    </div>
    """, unsafe_allow_html=True)
...
with kpi2:
    ...
    <div class="metric-card">
        ...
        <div class="metric-subtitle">{"Bonus extra applicato" if extra_invested > 0 else "Nessun extra sbloccato"}</div>
    </div>
    """, unsafe_allow_html=True)
...
with kpi3:
    ...
    <div class="metric-card">
        ...
        <div class="metric-subtitle">Distanza SMA20: {best_dist_sma:.2f}%</div>
    </div>
    """, unsafe_allow_html=True)
...
with kpi4:
    ...
    <div class="metric-card">
        ...
        <div class="metric-subtitle">RSI: {best_rsi:.1f}</div>
    </div>
    """, unsafe_allow_html=True)
...
st.subheader(f"💶 Assegnazione Ponderata: €{total_investable:.2f} su {selected_etf_name}")
...
headers = ["ETF / DETTAGLIO", "PRICE", "CHANGE", "RSI (14)", "BOLLINGER", "DIST. SMA20", "MACD", "PAC SIGNAL", "COMPRA (€)"]
...
for _, row in df_display.iterrows():
    ...
    dist_val = row['Dist_SMA']
    dist_class = "badge-green" if dist_val < 0 else "badge-red"
    c6.markdown(f"<span class='badge {dist_class}'>● {dist_val:.1f}%</span>", unsafe_allow_html=True)
...
<div class="insight-box">
    📊 <b>Logica di Allocazione Ponderata:</b> 
    Il capitale investito su <b>{selected_etf_name}</b> viene modulato in base alla combinazione di distanza dalla SMA20 e RSI. Più l'asset si trova in zona di sconto sotto la media, maggiore sarà la quota di budget e salvadanaio impiegata.
</div>
""", unsafe_allow_html=True)

st.caption("Source: Yahoo Finance Live Data · Developed Markets PAC Screener | QuantEdge v4.2")