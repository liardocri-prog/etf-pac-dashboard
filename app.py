...
def fetch_etf_data():
    ...
    for symbol, info in ETF_CONFIG.items():
        try:
            ...
            dist_sma = ((current_price - cur_sma_20) / cur_sma_20) * 100  # Distanza percentuale dalla media
            ...
            # Nuova logica personalizzata: Sotto la media e a ribasso (Bearish o Variazione Negativa)
            sotto_media = current_price < cur_sma_20
            a_ribasso = (macd_status == "Bearish") or (change_pct < 0)

            if sotto_media and a_ribasso:
                signal = "Strong Buy"
            ...
            data_list.append({
                ...
                "DD_Val": drawdown_val,
                "Dist_SMA": dist_sma, # Usato per ordinare chi sta più sotto la media
                "MACD": macd_status,
                "Signal": signal
            })
...
# --- LOGICA DI SELEZIONE: Priorità a chi è sotto la media e a ribasso (ordinato per Dist_SMA crescente) ---
...
with col_title:
    ...
    st.write("Acquisto mensile concentrato sull'ETF **sotto la media mobile e a ribasso**. Clicca sull'ETF per aprire i grafici.")
...
with kpi3:
    ...
    <div class="metric-card">
        ...
        <div class="metric-subtitle">Distanza SMA20: {df.loc[best_etf_idx, 'Dist_SMA']:.2f}%</div>
    </div>
    """, unsafe_allow_html=True)
...
<div class="insight-box">
    📊 <b>Strategia Sotto la Media e a Ribasso:</b> 
    Questo mese tutto il budget (€{total_investable:.2f}) viene allocato su <b>{selected_etf_name}</b> poiché si trova sotto la sua media mobile a 20 periodi e mostra un trend a ribasso (fase di storno ottimale per accumulare).
</div>
""", unsafe_allow_html=True)
